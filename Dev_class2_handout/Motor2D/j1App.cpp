...
bool j1App::Awake()
{
	...
	if (result) {
		LOG("File was loaded correctly");
	}

	else {
		LOG("File not loaded correctly");
		return false;
	}

	branch = config_doc.child("config").child("name");
	LOG(branch.attribute("value").as_string());

	branch = config_doc.child("config");
	...
	while(item != NULL && ret == true)
	{
		...
		// that can be used to read all variables from that section. Send nullptr if the section does not exist in config.xml
			...
			ret = item->data->Awake(&branch.child(item->data->name.GetString()));
			item = item->next;
